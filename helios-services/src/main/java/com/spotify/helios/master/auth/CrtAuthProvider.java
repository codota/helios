 ...
 */
public class CrtAuthProvider<T> implements InjectableProvider<Auth, Parameter> {
  private static final Logger LOGGER = LoggerFactory.getLogger(CrtAuthProvider.class);

  private static class CrtAuthInjectable<T> extends AbstractHttpContextInjectable<T> {
    private static final String AUTH_SCHEME = "X-CHAP";
   ...
   */
  public CrtAuthProvider(final Authenticator<String, T> authenticator) {
    this.authenticator = authenticator;
  }
